            ...
            case POST:
            {
                ...
                if (mData != null && mData.size() > 0)
                {
                    if (mData instanceof FormEntity)
                    {
                        ...
                        ((HttpPost) request).setEntity(new UrlEncodedFormEntity(postParams, HttpApi.ENCODING_UTF8));
                    }
                    else if (mData instanceof MultipartEntity)
                    {
                        ...
                        for (Map.Entry<String, Object> entry : mData.entrySet())
                        {
                            ...
                            if (value instanceof File)
                            {
                                ...
                                builder.addBinaryBody(entry.getKey(), file, ContentType.parse(HttpApi.MEDIA_TYPE_MULTIPART), entry.getKey());
                            }
