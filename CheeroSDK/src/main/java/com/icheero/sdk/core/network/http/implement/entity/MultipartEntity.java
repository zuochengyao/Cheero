...
public class MultipartEntity extends AbstractHttpEntity
{
    ...
    public void addString(String key, String value)
    {
        try
        {
            ...
            write(ENCODING_BIT, BaseApi.MEDIA_TYPE_TEXT, value.getBytes(BaseApi.ENCODING_UTF8), key, null);
        }
        catch (IOException e)
        {
            e.printStackTrace();
    ...
    public void addBinaryPart(String key, byte[] data)
    {
        ...
        {
            mMap.put(key, data);
            write(ENCODING_BINARY, BaseApi.MEDIA_TYPE_MULTIPART, data, key, key);
        }
        catch (IOException e)
        {
            e.printStackTrace();
    ...
    public void addFilePart(String key, File file)
    {
        ...
        {
            mMap.put(key, file);
            write(ENCODING_BINARY, BaseApi.MEDIA_TYPE_MULTIPART, FileUtils.getFileBytes(file), key, file.getName());
        }
        catch (IOException e)
        {
            e.printStackTrace();
    ...
    {
        // boundary line
        mOutputStream.write((DASH_DASH + mBoundary + NEW_LINE).getBytes(BaseApi.ENCODING_UTF8));
        // content-disposition line
        mOutputStream.write(getContentDisposition(key, fileName).getBytes(BaseApi.ENCODING_UTF8));
        // content-type line
        mOutputStream.write((HEADER_CONTENT_TYPE + contentType + NEW_LINE).getBytes(BaseApi.ENCODING_UTF8));
        // content-length line
        mOutputStream.write((HEADER_CONTENT_LENGTH + data.length + NEW_LINE).getBytes(BaseApi.ENCODING_UTF8));
        // content-encoding line
        mOutputStream.write((contentEncoding + NEW_LINE).getBytes(BaseApi.ENCODING_UTF8));
        // new line
        mOutputStream.write(NEW_LINE.getBytes(BaseApi.ENCODING_UTF8));
        // data line
